...
class SubcategoryService
{
    ...
    public function updateSubcategory($id, $data, $file = null, $userId = null, $isSuperAdmin = false)
    {
        try {
            ...
            // Check if category_id is being changed and handle cascading updates
            if (isset($data['category_id']) && $data['category_id'] != $subcategory['category_id']) {
                error_log("Subcategory category_id is being changed from {$subcategory['category_id']} to {$data['category_id']}");
                
                // Update all products in this subcategory to have the new category_id
                $this->updateProductsCategoryForSubcategory($id, $data['category_id'], $userId);
            }
    ...
    /**
     * Update category_id for all products in a subcategory when subcategory's parent category changes
     *
     * @param int $subcategoryId Subcategory ID
     ...
     * @return bool Success status
     */
    private function updateProductsCategoryForSubcategory($subcategoryId, $newCategoryId, $userId)
    {
        try {
            error_log("Starting updateProductsCategoryForSubcategory for subcategory ID: {$subcategoryId}, new category ID: {$newCategoryId}");
            
            // Get database instance
            $db = \App\Core\Database::getInstance();
            
            // First, get all products for this subcategory to log the changes
            $selectSql = "SELECT id, name, category_id FROM products WHERE subcategory_id = :subcategory_id AND deleted_at IS NULL";
            $selectParams = [':subcategory_id' => $subcategoryId];
            $products = $db->fetchAll($selectSql, $selectParams);
            
            error_log("Found " . count($products) . " products to update category_id for subcategory ID: {$subcategoryId}");
            
            if (!empty($products)) {
                // Update all products' category_id
                ...
                $updateSql .= " WHERE subcategory_id = :subcategory_id AND deleted_at IS NULL";
                ...
                // Execute the update query and get the statement
                ...
                if ($result) {
                    ...
                    error_log("Successfully updated category_id for {$affectedRows} products in subcategory {$subcategoryId}");
                    
                    // Log each product update for audit trail
                    foreach ($products as $product) {
                        error_log("Product ID: {$product['id']} ({$product['name']}) category_id changed from {$product['category_id']} to {$newCategoryId}");
                    }
                } else {
                    error_log("Failed to update products category_id for subcategory {$subcategoryId}");
                    return false;
                }
            } else {
                error_log("No products found for subcategory ID: {$subcategoryId}");
            ...
            error_log('Error in updateProductsCategoryForSubcategory: ' . $e->getMessage());
            error_log('Stack trace: ' . $e->getTraceAsString());
            return false;
        }
    }
