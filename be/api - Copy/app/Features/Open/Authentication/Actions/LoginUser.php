...
class LoginUser
{
   ...
   public function __invoke(Request $request, Response $response)
   {
       try {
           ...
           error_log("LoginUser - Request body: " . json_encode($requestBody));
           ...
           // Log what we found
           error_log("LoginUser - Phone number found: " . ($phoneNumber ?? 'null'));
           
           // Validate phone number
           if (empty($phoneNumber)) {
               return $response->json([
                   'status' => 'error',
                   'message' => 'Please enter your phone number to sign in to your account.',
                   'data' => null
               ]);
           }
           
           // Login user and send OTP
           $result = $this->authService->login($phoneNumber);
           
           // Check if this is a login or registration action
           $isRegistration = isset($result['action']) && $result['action'] === 'register';
           
           // Return appropriate success response based on action
           if ($isRegistration) {
               return $response->json([
                   'status' => 'success',
                   'message' => 'We noticed this is your first time here. A verification code has been sent to your phone to create your account. The code will expire in 2 minutes.',
                   'data' => $result
               ]);
           } else {
               return $response->json([
                   'status' => 'success',
                   'message' => 'A verification code has been sent to your phone. Please enter it to sign in to your account. The code will expire in 2 minutes.',
                   'data' => $result
               ]);
           }
